    ...
    ui(new Ui::Widget)
{
    ...
    typeModel->setQuery("SET NAMES 'utf8'");
...
void Widget::on_sellTypeComboBox_currentIndexChanged(QString type)
{
    if (type == "请选择类型") {
        ...
        sellBrandModel->setQuery("SET NAMES 'utf8'");
        sellBrandModel->setQuery(QString("select name from brand where type='%1'") .arg(type));
        ui->sellBrandComboBox->setModel(sellBrandModel); ui->sellCancelBtn->setEnabled(true);
    }
}
...
void Widget::on_goodsTypeComboBox_currentIndexChanged(QString type)
{
    if (type == "请选择类型") {
        ...
        goodBrandModel->setQuery("SET NAMES 'utf8'");
...
void Widget::on_sellBrandComboBox_currentIndexChanged(QString brand)
{
    ...
    query.next(); int num = query.value(0).toInt();
    if (num == 0) {
...
void Widget::on_goodsBrandComboBox_currentIndexChanged(QString brand)
{
    ...
    QSqlQuery query;
    query.exec(QString("select price from brand where name='%1' and type='%2'") .arg(brand).arg(ui->goodsTypeComboBox->currentText()));
    query.next();
...
void Widget::on_goodsOkBtn_clicked()
{
    ...
    QSqlQuery query;
    // 获取以前的总量
    query.exec(QString("select sum from brand where name='%1' and type='%2'") .arg(name).arg(type));
    query.next();
    int sum = query.value(0).toInt() + value;
    // 获取以前的剩余量
    query.exec(QString("select last from brand where name='%1' and type='%2'") .arg(name).arg(type));
    query.next();
    int last = query.value(0).toInt() + value;
    // 事务操作
    QSqlDatabase::database().transaction();
    bool rtn = query.exec( QString("update brand set sum=%1,last=%2 where name='%3' and type='%4'") .arg(sum).arg(last).arg(name).arg(type));
    if (rtn) {
        QSqlDatabase::database().commit();
...
void Widget::on_newOkBtn_clicked()
{
    ...
    //先判断品牌存不存在，如果已经存在该品牌，则不可插入
    QSqlQueryModel *sellBrandModel = new QSqlQueryModel(this);
    sellBrandModel->setQuery("SET NAMES 'utf8'");
    sellBrandModel->setQuery(QString("select name from brand where type='%1' and name='%2'") .arg(type).arg(brand));
    if(sellBrandModel->rowCount()>0){
        QMessageBox::warning(this, tr("提示"), tr("该类型已存在该品牌！"), QMessageBox::Ok);
    }else{
        ...
        // 事务操作
        QSqlDatabase::database().transaction();
        bool rtn = query.exec(QString("insert into brand values('%1', '%2', '%3', %4, %5, 0, %6)") .arg(id).arg(brand).arg(type).arg(price).arg(num).arg(num));
        if (rtn) {
            QSqlDatabase::database().commit();
            QMessageBox::information(this, tr("提示"), tr("入库成功！"), QMessageBox::Ok);
        } else {
...
{
    if (ui->oldPwdLineEdit->text().isEmpty() || ui->newPwdLineEdit->text().isEmpty()) {
        QMessageBox::warning(this, tr("警告"), tr("请将信息填写完整！"), QMessageBox::Ok);
    } else {
        QSqlQuery query; query.exec("select pwd from password");
        query.next();
        if (query.value(0).toString() == ui->oldPwdLineEdit->text()) {
            bool temp = query.exec(QString("update password set pwd='%1' where id='01'") .arg(ui->newPwdLineEdit->text()));
            if (temp) {
                QMessageBox::information(this, tr("提示"), tr("密码修改成功！"), QMessageBox::Ok);
                ui->oldPwdLineEdit->clear();
                ui->newPwdLineEdit->clear();
            } else {
                QMessageBox::information(this, tr("提示"), tr("密码修改失败，无法访问数据库！"), QMessageBox::Ok);
            }
        } else {
            QMessageBox::warning(this, tr("警告"), tr("原密码错误，请重新填写！"), QMessageBox::Ok);
            ui->oldPwdLineEdit->clear();
            ui->newPwdLineEdit->clear();
